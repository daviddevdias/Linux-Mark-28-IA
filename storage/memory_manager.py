...
MEMORY_PATH = pasta_raiz_app() / "api" / "long_term.json"
    ...
    with trava_memoria:
        ...
        try:
            ...
            if not isinstance(data, dict):
                return estrutura_memoria_vazia()
...
def get_nome():
    ...
    if cfg_n:
        return cfg_n
    return (
        load_memory().get("identity", {}).get("mestre", {}).get("value", "").strip()
        or "Usuário"
    )
...
def get_value(category: str, key: str, default: Any = None) -> Any:
    ...
    return node.get("value", default) if isinstance(node, dict) else node or default
